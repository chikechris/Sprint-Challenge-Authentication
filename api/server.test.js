...
describe('auth-route.js', () => {
  ...
  describe("GET /api/jokes", () => {
    ...
    it('should return return the object {you: Access Denied} when login in without token', () => {
      return request(server)
        .get('/api/jokes')
        .then(res => {
          expect(res.body).toEqual({ you: 'Access Denied' })
        })
    })
  })
})
...
describe("POST /api/auth/login", () => {
  ...
  describe('testing login endpoint', () => {
    it("should add a new user to the table", () => {
      return request(server)
        .post('/api/auth/register')
        .send({ username: 'chris', password: 'better' })
        .expect(201)
    })
    it('should return a 400 status when given an incorrect password', () => {
      return request(server)
        .post('/api/auth/login')
        .send({ username: 'chris', password: 'best' })
        .then(res => {
